...
if image_file is not None:
    ...
    if st.button("Illumination Normalization"):
        ...
        st.session_state["steps"]["illum_norm.jpg"] = illum_norm

    if st.button("Outline Image"):
        outlined = outline(image)
        display_image(outlined, "Outlined Image")
        st.session_state["steps"]["outlined.jpg"] = outlined

    # زر التحميل
    if st.session_state["steps"]:
        zip_buffer = io.BytesIO()
        with zipfile.ZipFile(zip_buffer, "w") as zip_file:
            for filename, img in st.session_state["steps"].items():
